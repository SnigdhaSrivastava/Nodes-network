...
export class NodePanelService  {
  constructor(private _interfaces: InterfaceinfoService) {
    this._interfaces.getInterfacesInfo().subscribe((data) => {
      interfaces = data;
  ...
  getNodePanel(nodes, links, width, height) {
    function newMethod(arg, arg1) {
      ...
      jsPanel.create({
        ...
        callback: function () {
          ...
          var node_interfaces = d3
            .select('#node_interfaces')
            .attr('align', 'center')
            .append('svg')
            .attr('width', 490)
            .attr('height', 80)
    ...
    function selectNode(selectedNode) {
      ...
      nodeElements.attr('stroke', function (node) {
        return getNeighborColor(node, neighbors);
      });
      textElements.attr('fill', function (node) {
        return getTextColor(node, neighbors);
      });
      linkElements.attr('stroke', function (d) {
        return getLinkColor(selectedNode, d);
      });
    }
    ...
    simulation.nodes(nodes).on('tick', () => {
      ...
      linkElements
        ...
        .attr('y1', function (link) {
          return link.source.y + 10;
        })
        .attr('x2', function (link) {
          return link.target.x + 10;
        })
        .attr('y2', function (link) {
          return link.target.y + 10;
        });
    });
